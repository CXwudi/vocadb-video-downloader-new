...
## Summary
Upgrade the multi-module Maven build from Spring Boot 3.5.10 to the latest Spring Boot 4.0.x GA, following the official upgrade guide first (including Jackson 3 migration), then run Kotest. If Kotest breaks, stop and resolve issue #118 first.
...
2. Bump Spring Boot to 4.x
- Update the root `spring-boot.version` to the latest 4.0.x GA.
- Ensure plugin alignment (spring-boot-maven-plugin uses the property already).
- Checkpoint: `./mvnw -DskipTests install` resolves dependencies (compile can still fail at this point).

3. Apply the official Spring Boot 4 upgrade guide
- Work through the official migration steps in order, including Spring Framework 7 changes and configuration updates.
...
- Checkpoint: apps start without migrator warnings; no migration-guide blockers remain.

4. Jackson 3 migration fixes (as required by the guide)
...
5. Kotest compatibility check (gating)
- Run a minimal Kotest + Spring test to verify SpringExtension still works with SB4.
...
- Kotest + SpringExtension compatibility test (gating, after migration work).
...
- Kotest is a hard gate after migration; failure triggers a stop and work on #118 before continuing.
