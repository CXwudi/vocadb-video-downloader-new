...
abstract class BaseCliAudioExtractor(
  ...
  override fun tryExtract(inputPvFile: Path, baseFileName: String, outputDirectory: Path): Path {
    // build commands
    val commands = buildCommands(inputPvFile, baseFileName, outputDirectory)

    // execute commands
    log.info { "Executing commands: ${commands.joinToString(" ", "`", "`")}" }
    executeCommands(commands)
    log.info { "Done command execution for $baseFileName, start finding the extracted audio file" }
    // find the extracted audio file
    return findExtractedAudioFile(outputDirectory, baseFileName)
  }
  ...
  private fun findExtractedAudioFile(outputDirectory: Path, baseFileName: String): Path {
    val possibleFiles =
      outputDirectory.listDirectoryEntries("*${baseFileName.replace("[", "\\[").replace("]", "\\]")}*")
    if (possibleFiles.isEmpty()) {
      throw IllegalStateException("No extracted audio file that contain $baseFileName as file name found in $outputDirectory")
    } else if (possibleFiles.size >= 2) {
      throw IllegalStateException("This should not happened, more than one file found contains $baseFileName.")
    } else {
